...
export default function AnalyticsPage() {
  ...
  const [year2_11, setYear2_11] = useState(2018);
  ...
  useEffect(() => {
    ...
    fetch(`http://${config.server_host}:${config.server_port}/analytics11?year1=${year1_11}&year2=${year2_11}`)
      .then(res => {return res.json()})
      .then(resJson => {
  ...
  return (
    <Container>
      ...
      />
      <h3>Which precincts exhibited the largest difference in votes between elections of years X and Y?</h3>
      <Dropdown className = 'Dropdown' placeholder='2016' options = {[2016, 2018, 2020]} onChange={(value) => { setYear1_11(value.value) }}/>
      <Dropdown className = 'Dropdown' placeholder='2018' options = {[2016, 2018, 2020]} onChange={(value) => { setYear2_11(value.value) }}/>
      <DataGrid
        rows={data11}
        columns={columns11}
        autoHeight
